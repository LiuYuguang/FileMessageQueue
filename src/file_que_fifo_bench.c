...
int bench_latency(int count){
    ...
    if(pid == 0){
        ...
        for(i=0;i<count;i++){
            tmp_len = read_file_que_timedwait_fifo("./tmp",&past,sizeof(past),-1);
        ...
        for(i=0;i<count;i++){
            now = _localtime();
            write_file_que_timedwait_fifo("./tmp",&now,sizeof(now),5);
            usleep(1000);
        }
...
int bench_read(int count){
    ...
    past = _localtime();
    for(i=0;i<count;i++){
        read_file_que_timedwait_fifo("./tmp",&now,sizeof(now),5);
    }
    now = _localtime();
